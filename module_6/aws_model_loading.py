...
def run_inference_test(pipeline, df_train, df_val):
    ...
    for i in range(min(10, len(y_val))):
        print(
            f"  {y_val.iloc[i]:6.2f} vs {y_pred[i]:6.2f} (diff: {abs(y_val.iloc[i] - y_pred[i]):5.2f})"
        )
...
if __name__ == "__main__":
    ...
    train_path = (
        "/Users/jordanharris/Code/mlops_zoomcamp/data/yellow_tripdata_2023-01.parquet"
    )
    val_path = (
        "/Users/jordanharris/Code/mlops_zoomcamp/data/yellow_tripdata_2023-02.parquet"
    )
    ...
    print(
        f"Model performance: {'Good' if results['r2'] > 0.5 else 'Needs improvement'} (R² = {results['r2']:.3f})"
    )
